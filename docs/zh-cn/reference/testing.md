...
EVO 提供两条测试路径：用于快速生成测试的内置 QA 智能体，以及用于企业级测试策略的可安装测试架构师模块。
...
| **设置** | 无需安装——包含在 BMM 中 | 通过 `npx evo-method install` 单独安装 |
...
- **文档：** [TEA 模块文档](https://evo-code-org.github.io/evo-method-test-architecture-enterprise/)
- **安装：** `npx evo-method install` 并选择 TEA 模块
...
Quinn 的 Automate 工作流出现在 EVO 方法工作流图的第 4 阶段（实现）。典型序列：
