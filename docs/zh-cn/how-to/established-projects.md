---
title: "既有项目"
description: 如何在现有代码库中使用 EVO Method
sidebar:
  order: 6
---

在现有项目和遗留代码库上工作时，有效使用 EVO Method。

本指南涵盖了使用 EVO Method 接入现有项目的核心工作流程。

:::note[前置条件]
- 已安装 EVO Method（`npx evo-method install`）
- 一个你想要处理的现有代码库
...
如果你通过 EVO 流程完成了所有 PRD 史诗和用户故事，请清理这些文件。归档它们、删除它们，或者在需要时依赖版本历史。不要将这些文件保留在：
...
你可以查看和完善生成的文件，或者如果你更喜欢，可以在 `_evo-output/project-context.md` 手动创建它。
...
**随时运行 `/evo-help`，当你不确定下一步该做什么时。** 这个智能指南：
...
```
/evo-help 我有一个现有的 Rails 应用，我应该从哪里开始？
/evo-help quick-flow 和完整方法有什么区别？
/evo-help 显示我有哪些可用的工作流程
```

EVO-Help 还会在**每个工作流程结束时自动运行**，提供关于下一步该做什么的清晰指导。
...
| 范围                          | 推荐方法                                                                                                          |
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------- |
| **小型更新或添加** | 使用 `quick-flow-solo-dev` 创建技术规范并实施变更。完整的四阶段 EVO Method 可能有些过度。 |
| **重大变更或添加** | 从 EVO Method 开始，根据需要应用或多或少的严谨性。                                                    |
...
- **EVO Method**：EVO 方法。一种结构化的软件开发方法论，用于指导从分析到实施的完整流程。
...
- **quick-flow**：快速流程。EVO Method 中的一种简化工作流程，适用于小型变更或快速迭代。
