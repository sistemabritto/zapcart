---
title: "如何升级到 v6"
description: 从 EVO v4 迁移到 v6
sidebar:
  order: 3
---

使用 EVO 安装程序从 v4 升级到 v6，其中包括自动检测旧版安装和迁移辅助。

## 何时使用本指南

- 您已安装 EVO v4（`.evo-method` 文件夹）
- 您希望迁移到新的 v6 架构
- 您有需要保留的现有规划产物
...
按照[安装程序说明](./install-evo.md)操作。

### 2. 处理旧版安装

当检测到 v4 时，您可以：

- 允许安装程序备份并删除 `.evo-method`
- 退出并手动处理清理

如果您将 evo method 文件夹命名为其他名称 - 您需要手动删除该文件夹。

### 3. 清理 IDE 命令

手动删除旧版 v4 IDE 命令 - 例如如果您使用 claude，查找任何以 evo 开头的嵌套文件夹并删除它们：

- `.claude/commands/EVO/agents`
- `.claude/commands/EVO/tasks`

### 4. 迁移规划产物

**如果您有规划文档（Brief/PRD/UX/Architecture）：**

将它们移动到 `_evo-output/planning-artifacts/` 并使用描述性名称：
...
1. 完成 v6 安装
2. 将 `epics.md` 或 `epics/epic*.md` 放入 `_evo-output/planning-artifacts/`
...
└── _evo-output/        # 输出文件夹（v4 中为 doc 文件夹）
```
...
| v4 模块                       | v6 状态                                   |
| ----------------------------- | ----------------------------------------- |
| `.evo-2d-phaser-game-dev`    | 已集成到 BMGD 模块                        |
| `.evo-2d-unity-game-dev`     | 已集成到 BMGD 模块                        |
| `.evo-godot-game-dev`        | 已集成到 BMGD 模块                        |
| `.evo-infrastructure-devops` | 已弃用 — 新的 DevOps 智能体即将推出       |
| `.evo-creative-writing`      | 未适配 — 新的 v6 模块即将推出             |

## 主要变更

| 概念         | v4                                      | v6                                   |
| ------------ | --------------------------------------- | ------------------------------------ |
| **核心**     | `_evo-core` 实际上是 EVO Method      | `_evo/core/` 是通用框架             |
| **方法**     | `_evo-method`                          | `_evo/bmm/`                         |
...
- **BMGD**：EVO Game Development。EVO 游戏开发模块。
- **DevOps**：开发运维（Development Operations）。结合开发和运维的实践，旨在缩短系统开发生命周期。
- **EVO Method**：EVO 方法。EVO 框架的核心方法论模块。
- **EVO Builder**：EVO 构建器。EVO 框架的构建工具。
- **Creative Intelligence Suite**：创意智能套件。EVO 框架中的创意工具集合。
