...
直接使用 **DEV 智能体**进行 bug 修复、重构或小型针对性更改，这些操作不需要完整的 EVO Method 或 Quick Flow。
...
```text
/evo-agent-bmm-quick-flow-solo-dev
```
...
在以下情况下考虑使用 [Quick Flow](../explanation/quick-flow.md) 或完整的 EVO Method：
...
- **Quick Flow**：快速流程。EVO Method 中的一种工作流程，用于快速实现小型更改。
