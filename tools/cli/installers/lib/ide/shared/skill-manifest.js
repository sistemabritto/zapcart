...
/**
 * Load evo-skill-manifest.yaml from a directory.
 ...
 * @param {string} dirPath - Directory to check for evo-skill-manifest.yaml
 * @returns {Object|null} Parsed manifest or null
 */
async function loadSkillManifest(dirPath) {
  const manifestPath = path.join(dirPath, 'evo-skill-manifest.yaml');
  try {
    ...
    console.warn(`Warning: Failed to parse evo-skill-manifest.yaml in ${dirPath}: ${error.message}`);
    return null;
  }
}
...
/**
 * Get the install_to_evo flag for a specific file from a loaded skill manifest.
 * @param {Object|null} manifest - Loaded manifest (from loadSkillManifest)
 * @param {string} filename - Source filename to look up
 * @returns {boolean} install_to_evo value (defaults to true)
 */
function getInstallToEvo(manifest, filename) {
  if (!manifest) return true;
  // Single-entry manifest applies to all files in the directory
  if (manifest.__single) return manifest.__single.install_to_evo !== false;
  // Multi-entry: look up by filename directly
  if (manifest[filename]) return manifest[filename].install_to_evo !== false;
  // Fallback: try alternate extensions for compiled files
  const baseName = filename.replace(/\.(md|xml)$/i, '');
  const agentKey = `${baseName}.agent.yaml`;
  if (manifest[agentKey]) return manifest[agentKey].install_to_evo !== false;
  const xmlKey = `${baseName}.xml`;
  if (manifest[xmlKey]) return manifest[xmlKey].install_to_evo !== false;
  return true;
}

module.exports = { loadSkillManifest, getCanonicalId, getArtifactType, getInstallToEvo };
