...
/**
 * Helpers for gathering EVO agents/tasks from the installed tree.
 * Shared by installers that need Claude-style exports.
 */
async function getAgentsFromEvo(evoDir, selectedModules = []) {
  const agents = [];

  // Get core agents
  if (await fs.pathExists(path.join(evoDir, 'core', 'agents'))) {
    const coreAgents = await getAgentsFromDir(path.join(evoDir, 'core', 'agents'), 'core');
    agents.push(...coreAgents);
  }

  // Get module agents
  for (const moduleName of selectedModules) {
    const agentsPath = path.join(evoDir, moduleName, 'agents');
...
async function getTasksFromEvo(evoDir, selectedModules = []) {
  const tasks = [];

  if (await fs.pathExists(path.join(evoDir, 'core', 'tasks'))) {
    const coreTasks = await getTasksFromDir(path.join(evoDir, 'core', 'tasks'), 'core');
    tasks.push(...coreTasks);
  }

  for (const moduleName of selectedModules) {
    const tasksPath = path.join(evoDir, moduleName, 'tasks');
