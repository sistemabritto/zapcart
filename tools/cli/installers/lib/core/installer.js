...
class Installer {
  constructor() {
    ...
    this.evoFolderName = EVO_FOLDER_NAME;
  }

  /**
   * Find the evo installation directory in a project
   * Always uses the standard _evo folder name
   * Also checks for legacy _cfg folder for migration
   * @param {string} projectDir - Project directory
   * @returns {Promise<Object>} { evoDir: string, hasLegacyCfg: boolean }
   */
  async findEvoDir(projectDir) {
    const evoDir = path.join(projectDir, EVO_FOLDER_NAME);

    // Check if project directory exists
    if (!(await fs.pathExists(projectDir))) {
      // Project doesn't exist yet, return default
      return { evoDir, hasLegacyCfg: false };
    }

    // Check for legacy _cfg folder if evo directory exists
    let hasLegacyCfg = false;
    if (await fs.pathExists(evoDir)) {
      const legacyCfgPath = path.join(evoDir, '_cfg');
      if (await fs.pathExists(legacyCfgPath)) {
        hasLegacyCfg = true;
      }
    }

    return { evoDir, hasLegacyCfg };
  }

  /**
   * @function copyFileWithPlaceholderReplacement
   * @intent Copy files from EVO source to installation directory with dynamic content transformation
   * @why Enables installation-time customization: _evo replacement
   * @param {string} sourcePath - Absolute path to source file in EVO repository
   * @param {string} targetPath - Absolute path to destination file in user's project
   * @param {string} evoFolderName - User's chosen evo folder name (default: 'evo')
  ...
  async collectToolConfigurations(
    ...
    // During full reinstall, use the saved previous IDEs since evo dir was deleted
    ...
    const savedIdeConfigs = await this.ideConfigManager.loadAllIdeConfigs(evoDir);
  ...
  async install(originalConfig) {
    ...
    if (!hasCoreConfig) {
      ...
      await CLIUtils.displaySection('EVO™  Installation', 'Version ' + require(path.join(getProjectRoot(), 'package.json')).version);
    }
      ...
      if (config._isUpdate && config._existingInstall && config._existingInstall.customModules) {
        for (const customModule of config._existingInstall.customModules) {
          ...
          if (absoluteSourcePath && absoluteSourcePath.startsWith('_config')) {
            ...
            absoluteSourcePath = path.join(evoDir, absoluteSourcePath);
          }
          // If no sourcePath but we have relativePath, convert it
          else if (!absoluteSourcePath && customModule.relativePath) {
            // relativePath is relative to the project root (parent of evo dir)
    ...
    // Set evo folder name on module manager and IDE manager for placeholder replacement
    this.moduleManager.setEvoFolderName(EVO_FOLDER_NAME);
    this.moduleManager.setCoreConfig(moduleConfigs.core || {});
    this.moduleManager.setCustomModulePaths(customModulePaths);
    this.ideManager.setEvoFolderName(EVO_FOLDER_NAME);
    ...
    try {
      ...
      const existingInstall = await this.detector.detect(evoDir);
      ...
      if (existingInstall.installed && !config.force && !config._quickUpdate) {
        ...
        if (config.actionType === 'update') {
          ...
          await prompts.log.warn('Existing EVO installation detected');
          await prompts.log.message(`  Location: ${evoDir}`);
        ...
        if (action === 'update') {
          ...
          if (modulesToRemove.length > 0) {
            if (config.skipPrompts) {
              ...
              const confirmRemoval = await prompts.confirm({
                message: `Remove ${modulesToRemove.length} module(s) from EVO installation?`,
                default: false,
              });
          ...
          const existingFilesManifest = await this.readFilesManifest(evoDir);
          const { customFiles, modifiedFiles } = await this.detectCustomFiles(evoDir, existingFilesManifest);
          ...
          const coreConfigPath = path.join(evoDir, 'core', 'config.yaml');
          ...
          if (customFiles.length > 0) {
            const tempBackupDir = path.join(projectDir, '_evo-custom-backup-temp');
            await fs.ensureDir(tempBackupDir);

            spinner.start(`Backing up ${customFiles.length} custom files...`);
            for (const customFile of customFiles) {
              const relativePath = path.relative(evoDir, customFile);
          ...
          if (modifiedFiles.length > 0) {
            const tempModifiedBackupDir = path.join(projectDir, '_evo-modified-backup-temp');
            await fs.ensureDir(tempModifiedBackupDir);

            spinner.start(`Backing up ${modifiedFiles.length} modified files...`);
            for (const modifiedFile of modifiedFiles) {
              const relativePath = path.relative(evoDir, modifiedFile.path);
      ...
      if (config.ides && config.ides.length > 0) {
        ...
        if (suspendedIdes.length > 0 && suspendedIdes.length === config.ides.length) {
          ...
          throw new Error(
            `All selected tool(s) are suspended: ${suspendedIdes.join(', ')}. Installation aborted to prevent upgrading _evo/ without a working IDE configuration.`,
          );
        }
      }
      ...
      if (config._isUpdate && config._existingInstall) {
        ...
        if (idesToRemove.length > 0) {
          if (config.skipPrompts) {
            ...
            const confirmRemoval = await prompts.confirm({
              message: `Remove EVO configuration for ${idesToRemove.length} IDE(s)?`,
              default: false,
            });
            ...
            if (confirmRemoval) {
              ...
              for (const ide of idesToRemove) {
                try {
                  ...
                  await this.ideConfigManager.deleteIdeConfig(evoDir, ide);
      ...
      await this.createDirectoryStructure(evoDir);
      ...
      if (customModulePaths && customModulePaths.size > 0) {
        ...
        const customCache = new CustomModuleCache(evoDir);
      ...
      // Custom modules are already installed in _evo and don't need dependency resolution from source
      ...
      if (config.installCore) {
        installTasks.push({
          title: isQuickUpdate ? 'Updating EVO core' : 'Installing EVO core',
          task: async (message) => {
            await this.installCoreWithDependencies(evoDir, { core: {} });
            addResult('Core', 'ok', isQuickUpdate ? 'updated' : 'installed');
            await this.generateModuleConfigs(evoDir, { core: config.coreConfig || {} });
          ...
          task: async (message) => {
            ...
            for (const moduleName of allModules) {
              ...
              if (!isCustomModule && config._customModuleSources && config._customModuleSources.has(moduleName)) {
                ...
                if (customInfo.sourcePath && !customInfo.path) {
                  customInfo.path = path.isAbsolute(customInfo.sourcePath)
                    ...
                    : path.join(evoDir, customInfo.sourcePath);
                }
              }
              ...
              if (isCustomModule && customInfo) {
                ...
                await this.generateModuleConfigs(evoDir, {
                ...
                if (moduleName === 'core') {
                  await this.installCoreWithDependencies(evoDir, resolution.byModule[moduleName]);
                } else {
                  await this.installModuleWithDependencies(moduleName, evoDir, resolution.byModule[moduleName]);
                }
              }
            ...
            for (const [module, files] of Object.entries(resolution.byModule)) {
              if (!allModules.includes(module) && module !== 'core') {
                ...
                if (totalFiles > 0) {
                  ...
                  await this.installPartialModule(module, evoDir, files);
                }
              }
            }
      ...
      installTasks.push({
        ...
        task: async (message) => {
          ...
          const verboseMode = process.env.EVO_VERBOSE_INSTALL === 'true' || config.verbose;
          const moduleLogger = {
          ...
          if (config.installCore || resolution.byModule.core) {
            const result = await this.moduleManager.createModuleDirectories('core', evoDir, {
          ...
          if (config.modules && config.modules.length > 0) {
            for (const moduleName of config.modules) {
              ...
              const result = await this.moduleManager.createModuleDirectories(moduleName, evoDir, {
                installedIDEs: config.ides || [],
      ...
      const configTask = {
        ...
        task: async (message) => {
          ...
          await this.generateModuleConfigs(evoDir, moduleConfigs);
          ...
          const manifestStats = await manifestGen.generateManifests(evoDir, allModulesForManifest, [...this.installedFiles], {
            ides: config.ides || [],
          ...
          await this.mergeModuleHelpCatalogs(evoDir);
        ...
        if (validIdes.length === 0) {
          ...
          try {
            for (const ide of validIdes) {
              ...
              try {
                const setupResult = await this.ideManager.setup(ide, projectDir, evoDir, {
                ...
                if (ideConfigurations[ide] && !ideConfigurations[ide]._alreadyConfigured) {
                  await this.ideConfigManager.saveIdeConfig(evoDir, ide, ideConfigurations[ide]);
                }
          ...
          task: async (message) => {
            ...
            if (config._customFiles && config._customFiles.length > 0) {
              ...
              for (const originalPath of config._customFiles) {
                const relativePath = path.relative(evoDir, originalPath);
   ...
   * @param {Object} context - {evoDir, modules, ides, customFiles, modifiedFiles}
   */
  async renderInstallSummary(results, context = {}) {
    ...
    lines.push('');
    if (context.evoDir) {
      lines.push(`  Installed to: ${color.dim(context.evoDir)}`);
    }
    ...
    lines.push(
      '',
      '  Next steps:',
      `    Read our new Docs Site: ${color.dim('https://docs.evo-method.org/')}`,
      `    Join our Discord: ${color.dim('https://discord.gg/gk8jAdXWmj')}`,
      `    Star us on GitHub: ${color.dim('https://github.com/evo-code-org/EVO-METHOD/')}`,
      `    Subscribe on YouTube: ${color.dim('https://www.youtube.com/@EVOCode')}`,
      `    Run ${color.cyan('/evo-help')} with your IDE Agent and ask it how to get started`,
    );
  ...
  async update(config) {
    ...
    try {
      ...
      if (!existingInstall.installed) {
        spinner.stop('No EVO installation found');
        throw new Error(`No EVO installation found at ${evoDir}`);
      }
      ...
      if (existingInstall.hasCore) {
        ...
        await this.updateCore(evoDir, config.force);
      }

      for (const module of existingInstall.modules) {
        spinner.message(`Updating module: ${module.id}...`);
        await this.moduleManager.update(module.id, evoDir, config.force, { installer: this });
      }

      // Update manifest
      spinner.message('Updating manifest...');
      await this.manifest.update(evoDir, {
        version: newVersion,
  ...
  async getStatus(directory) {
    ...
    const { evoDir } = await this.findEvoDir(projectDir);
    return await this.detector.detect(evoDir);
  }
  ...
  /**
   * Uninstall EVO with selective removal options
   * @param {string} directory - Project directory
   * @param {Object} options - Uninstall options
   * @param {boolean} [options.removeModules=true] - Remove _evo/ directory
  ...
  async uninstall(directory, options = {}) {
    ...
    const existingInstall = await this.detector.detect(evoDir);
    const outputFolder = await this._readOutputFolder(evoDir);

    const removed = { modules: false, ideConfigs: false, outputFolder: false };

    // 2. IDE CLEANUP (before _evo/ deletion so configs are accessible)
    ...
    // 4. EVO DIRECTORY (last, after everything that needs it)
   ...
   * Resolves evoDir internally from projectDir
   * @param {string} projectDir - Project directory
   * @returns {string} Output folder name (relative, default: '_evo-output')
   */
  async getOutputFolder(projectDir) {
    const { evoDir } = await this.findEvoDir(projectDir);
    return this._readOutputFolder(evoDir);
  }
   ...
   */
  async _readOutputFolder(evoDir) {
    const yaml = require('yaml');

    // Check bmm/config.yaml first (most common)
    const bmmConfigPath = path.join(evoDir, 'bmm', 'config.yaml');
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        const configPath = path.join(evoDir, entry.name, 'config.yaml');
   ...
   */
  /**
   * Merge all module-help.csv files into a single evo-help.csv
   * Scans all installed modules for module-help.csv and merges them
   * Enriches agent info from agent-manifest.csv
   * Output is written to _evo/_config/evo-help.csv
   * @param {string} evoDir - EVO installation directory
   */
  async mergeModuleHelpCatalogs(evoDir) {
    const allRows = [];
    const headerRow =
      'module,phase,name,code,sequence,workflow-file,command,required,agent-name,agent-command,agent-display-name,agent-title,options,description,output-location,outputs';

    // Load agent manifest for agent info lookup
    const agentManifestPath = path.join(evoDir, '_config', 'agent-manifest.csv');
    ...
    if (await fs.pathExists(agentManifestPath)) {
      ...
      for (const line of lines) {
        ...
        if (cols.length >= 4) {
          ...
          // Build agent command: evo:module:agent:name
          const agentCommand = module ? `evo:${module}:agent:${agentName}` : `evo:agent:${agentName}`;
    ...
    for (const moduleName of installedModules) {
      const modulePath = path.join(evoDir, moduleName);
    ...
    const outputDir = path.join(evoDir, '_config');
    await fs.ensureDir(outputDir);
    const outputPath = path.join(outputDir, 'evo-help.csv');
    ...
    if (process.env.EVO_VERBOSE_INSTALL === 'true') {
      await prompts.log.message(`  Generated evo-help.csv: ${allRows.length} workflows`);
    }
  }
  ...
  async createDirectoryStructure(evoDir) {
    await fs.ensureDir(evoDir);
    await fs.ensureDir(path.join(evoDir, '_config'));
    await fs.ensureDir(path.join(evoDir, '_config', 'agents'));
    await fs.ensureDir(path.join(evoDir, '_config', 'custom'));
  }
   ...
   */
  async generateModuleConfigs(evoDir, moduleConfigs) {
    const yaml = require('yaml');
...
`;
   ...
   */
  async installCoreWithDependencies(evoDir, coreFiles) {
    const sourcePath = getModulePath('core');
    const targetPath = path.join(evoDir, 'core');
    await this.installCore(evoDir);
  }
   ...
   */
  async installModuleWithDependencies(moduleName, evoDir, moduleFiles) {
   ...
   */
  async installPartialModule(moduleName, evoDir, files) {
    const sourceBase = getModulePath(moduleName);
    const targetBase = path.join(evoDir, moduleName);
  ...
  async installCore(evoDir) {
    ...
    await moduleManager.compileModuleAgents(sourcePath, targetPath, 'core', evoDir, this);
   ...
   * @param {string} modulePath - Path to module in evo/ installation
   * @param {string} moduleName - Module name
   */
  async processAgentFiles(modulePath, moduleName) {
    ...
    // Determine project directory (parent of evo/ directory)
    const evoDir = path.dirname(modulePath);
   ...
   * Private: Update core
   */
  async updateCore(evoDir, force = false) {
    const sourcePath = getModulePath('core');
    const targetPath = path.join(evoDir, 'core');
  ...
  async quickUpdate(config) {
    ...
    try {
      ...
      if (!(await fs.pathExists(evoDir))) {
        spinner.stop('No EVO installation found');
        throw new Error(`EVO not installed at ${evoDir}. Use regular install for first-time setup.`);
      }
  ...
  async compileAgents(config) {
    ...
    try {
      ...
      const moduleManager = new ModuleManager();
      moduleManager.setEvoFolderName(path.basename(evoDir));
      ...
      for (const moduleId of installedModules) {
        ...
        const targetPath = path.join(evoDir, moduleId);

        // Compile agents for this module
        await moduleManager.compileModuleAgents(sourcePath, targetPath, moduleId, evoDir, this);
  ...
  /**
   * Handle legacy EVO v4 detection with simple warning
  ...
  async handleLegacyV4Migration(_projectDir, _legacyV4) {
    await prompts.note(
      'Found .evo-method folder from EVO v4 installation.\n\n' +
        'Before continuing with installation, we recommend:\n' +
        '  1. Remove the .evo-method folder, OR\n' +
        '  2. Back it up by renaming it to another name (e.g., evo-method-backup)\n\n' +
        'If your v4 installation set up rules or commands, you should remove those as well.',
      'Legacy EVO v4 detected',
    );
    ...
    if (proceed === 'exit') {
      await prompts.log.info('Please remove the .evo-method folder and any v4 rules/commands, then run the installer again.');
   ...
   */
  async readFilesManifest(evoDir) {
    const filesManifestPath = path.join(evoDir, '_config', 'files-manifest.csv');
   ...
   */
  async detectCustomFiles(evoDir, existingFilesManifest) {
    const customFiles = [];
    const modifiedFiles = [];

    // Memory is always in _evo/_memory
    const evoMemoryPath = '_memory';
    ...
    for (const fileEntry of existingFilesManifest) {
      if (fileEntry.path) {
        const absolutePath = path.join(evoDir, fileEntry.path);
    ...
    // Recursively scan evoDir for all files
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            const relativePath = path.relative(evoDir, fullPath);
            ...
            if (relativePath.startsWith(evoMemoryPath + '/') && path.dirname(relativePath).includes('-sidecar')) {
              continue;
            }
   ...
   * @param {string} evoDir - EVO directory
   ...
   */
  async handleMissingCustomSources(customModuleSources, evoDir, projectRoot, operation, installedModules, skipPrompts = false) {
    ...
    for (const missing of customModulesWithMissingSources) {
      ...
      switch (action) {
        case 'update': {
          ...
          await this.manifest.addCustomModule(evoDir, missing.info);
        ...
        case 'remove': {
          ...
          if (confirmDelete) {
            ...
            if (typedConfirm === 'DELETE') {
              ...
              const modulePath = path.join(evoDir, missing.id);
              if (await fs.pathExists(modulePath)) {
              ...
              await this.manifest.removeModule(evoDir, missing.id);
              await this.manifest.removeCustomModule(evoDir, missing.id);
