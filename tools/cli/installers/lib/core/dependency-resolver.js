...
/**
 * Dependency Resolver for EVO modules
...
class DependencyResolver {
   ...
   */
  async resolve(evoDir, selectedModules = [], options = {}) {
    if (options.verbose) {
    ...
    const primaryFiles = await this.collectPrimaryFiles(evoDir, modulesToProcess, options);
    ...
    const resolvedDeps = await this.resolveDependencyPaths(evoDir, allDependencies);

    // Fourth pass: check for transitive dependencies
    const transitiveDeps = await this.resolveTransitiveDependencies(evoDir, resolvedDeps);

    // Combine all files
    const allFiles = new Set([...primaryFiles.map((f) => f.path), ...resolvedDeps, ...transitiveDeps]);

    // Organize by module
    const organizedFiles = this.organizeByModule(evoDir, allFiles);
   ...
   */
  async collectPrimaryFiles(evoDir, modules, options = {}) {
    const files = [];
    ...
    for (const module of modules) {
      ...
      // Handle both source (src/) and installed (evo/) directory structures
      let moduleDir;

      // Check if this is a source directory (has 'src' subdirectory)
      const srcDir = path.join(evoDir, 'src');
  ...
  async parseDependencies(files) {
    ...
    for (const file of files) {
      ...
      for (const ref of fileRefs) {
        ...
        // Paths starting with evo/ are absolute references to the evo installation
        const depType = ref.startsWith('evo/') ? 'evo-path' : 'file';
        allDeps.add({
          from: file.path,
          dependency: ref,
          type: depType,
  ...
  parseCommandReferences(content) {
    ...
    const commandPattern = /@(task-|agent-|evo-)([a-z0-9-]+)/g;
    let match;
    ...
    // Match file paths like evo/core/agents/analyst
    const pathPattern = /evo\/(core|bmm|cis)\/(agents|tasks)\/([a-z0-9-]+)/g;
  ...
  parseFileReferences(content) {
    ...
    while ((match = execPattern.exec(content)) !== null) {
      ...
      if (execPath && execPath !== '*') {
        ...
        // Usage is like {project-root}/evo/core/tasks/foo.md
   ...
   */
  async resolveDependencyPaths(evoDir, dependencies) {
    const resolved = new Set();

    for (const dep of dependencies) {
      const resolvedPaths = await this.resolveSingleDependency(evoDir, dep);
   ...
   */
  async resolveSingleDependency(evoDir, dep) {
    const paths = [];
    ...
    switch (dep.type) {
      ...
      case 'file': {
        ...
        if (depPath.includes('{project-root}')) {
          // Remove {project-root} and resolve as evo path
          depPath = depPath.replace('{project-root}', '');

          if (depPath.startsWith('evo/')) {
            const evoPath = depPath.replace(/^evo\//, '');

            // Handle glob patterns
            if (depPath.includes('*')) {
              // Extract the base path and pattern
              const pathParts = evoPath.split('/');
              ...
              let basePath;
              if (module === 'core') {
                basePath = path.join(evoDir, 'core', middlePath);
              } else {
                basePath = path.join(evoDir, 'modules', module, middlePath);
              }
              ...
              // Direct path
              if (evoPath.startsWith('core/')) {
                const corePath = path.join(evoDir, evoPath);
                if (await fs.pathExists(corePath)) {
                  paths.push(corePath);
                }
              } else {
                const parts = evoPath.split('/');
                const module = parts[0];
                const rest = parts.slice(1).join('/');
                const modulePath = path.join(evoDir, 'modules', module, rest);
      ...
      case 'command': {
        ...
        const commandPath = await this.resolveCommandToPath(evoDir, dep.dependency);
        if (commandPath) {
      ...
      case 'evo-path': {
        // Resolve evo/ paths (from {project-root}/evo/... references)
        // These are paths relative to the src directory structure
        const evoPath = dep.dependency.replace(/^evo\//, '');

        // Try to resolve as if it's in src structure
        // evo/core/tasks/foo.md -> src/core/tasks/foo.md
        // evo/bmm/tasks/bar.md -> src/bmm/tasks/bar.md (bmm is directly under src/)
        // evo/cis/agents/bar.md -> src/modules/cis/agents/bar.md
        ...
        if (evoPath.startsWith('core/')) {
          ...
          if (module === 'bmm') {
            ...
            modulePath = path.join(evoDir, module, rest);
          } else {
            // Other modules are under modules/
            modulePath = path.join(evoDir, 'modules', module, rest);
          }
      ...
      case 'template': {
        ...
        if (templateDep.includes('{project-root}')) {
          // Remove {project-root} and treat as evo-path
          templateDep = templateDep.replace('{project-root}', '');

          // Now resolve as a evo path
          if (templateDep.startsWith('evo/')) {
            const evoPath = templateDep.replace(/^evo\//, '');
   ...
   */
  async resolveCommandToPath(evoDir, command) {
    // Parse command format: @task-name or @agent-name or evo/module/type/name
    ...
    if (command.startsWith('@task-')) {
      ...
      for (const module of ['core', 'bmm', 'cis']) {
        const taskPath =
          module === 'core'
            ? path.join(evoDir, 'core', 'tasks', `${taskName}.md`)
            : path.join(evoDir, 'modules', module, 'tasks', `${taskName}.md`);
      ...
      for (const module of ['core', 'bmm', 'cis']) {
        const agentPath =
          module === 'core'
            ? path.join(evoDir, 'core', 'agents', `${agentName}.md`)
            : path.join(evoDir, 'modules', module, 'agents', `${agentName}.md`);
        if (await fs.pathExists(agentPath)) {
          return agentPath;
        }
      }
    } else if (command.startsWith('evo/')) {
      ...
      if (parts.length >= 4) {
        ...
        const filePath =
          module === 'core' ? path.join(evoDir, 'core', type, fileName) : path.join(evoDir, 'modules', module, type, fileName);
   ...
   */
  async resolveTransitiveDependencies(evoDir, directDeps) {
        ...
        const subDeps = await this.parseDependencies([
          {
            path: depPath,
            type: 'dependency',
            module: this.getModuleFromPath(evoDir, depPath),
            name: path.basename(depPath),
          },
        ]);

        const resolvedSubDeps = await this.resolveDependencyPaths(evoDir, subDeps);
   ...
   */
  getModuleFromPath(evoDir, filePath) {
    const relative = path.relative(evoDir, filePath);
   ...
   */
  organizeByModule(evoDir, files) {
    const organized = {};

    for (const file of files) {
      const module = this.getModuleFromPath(evoDir, file);
      if (!organized[module]) {
        organized[module] = {
          agents: [],
          tasks: [],
      ...
      if (file.includes('/src/core/') || file.includes('/src/bmm/')) {
        if (module === 'core') {
          moduleBase = path.join(evoDir, 'src', 'core');
        } else if (module === 'bmm') {
          moduleBase = path.join(evoDir, 'src', 'bmm');
        }
      } else {
        moduleBase = module === 'core' ? path.join(evoDir, 'core') : path.join(evoDir, 'modules', module);
      }
