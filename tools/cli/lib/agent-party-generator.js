...
const AgentPartyGenerator = {
  ...
  generateAgentParty(agentDetails, options = {}) {
    ...
    // Build XML content
    let xmlContent = `<!-- Powered by EVO-CORE™ -->
<!-- Agent Manifest - Generated during EVO ${forWeb ? 'bundling' : 'installation'} -->
<!-- This file contains a summary of all ${forWeb ? 'bundled' : 'installed'} agents for quick reference -->
<manifest id="evo/_config/agent-manifest.csv" version="1.0" generated="${new Date().toISOString()}">
  <description>
    Complete roster of ${forWeb ? 'bundled' : 'installed'} EVO agents with summarized personas for efficient multi-agent orchestration.
...
`;
  ...
  extractAgentDetails(content, moduleName, agentName) {
    try {
      ...
      return {
        id: `evo/${moduleName}/agents/${agentName}.md`,
