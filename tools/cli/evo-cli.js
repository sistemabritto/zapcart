  ...
  try {
    ...
    if (result && semver.gt(result, packageJson.version)) {
      ...
      const updateMsg = [
        ...
        `  npm cache clean --force && npx evo-method@${tag} install`,
      ].join('\n');
...
// Set up main program
program.version(packageJson.version).description('EVO Core CLI - Universal AI agent framework');
