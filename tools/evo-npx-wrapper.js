...
/**
 * EVO Method CLI - Direct execution wrapper for npx
...
if (isNpxExecution) {
  ...
  const evoCliPath = path.join(__dirname, 'cli', 'evo-cli.js');

  if (!fs.existsSync(evoCliPath)) {
    console.error('Error: Could not find evo-cli.js at', evoCliPath);
  ...
  try {
    ...
    execSync(`node "${evoCliPath}" ${args.join(' ')}`, {
      stdio: 'inherit',
