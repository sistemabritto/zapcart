...
test('bmm-style.csv: extracts workflow-file refs with trailing commas', () => {
  ...
  assert(refs[0].raw === '_evo/bmm/workflows/document-project/workflow.md', `Wrong raw[0]: ${refs[0].raw}`);
  assert(refs[1].raw === '_evo/core/workflows/brainstorming/workflow.md', `Wrong raw[1]: ${refs[1].raw}`);
...
test('core-style.csv: extracts refs from core module-help format', () => {
  ...
  const refs = extractCsvRefs(fullPath, content);
  assert(refs.length === 2, `Expected 2 refs, got ${refs.length}`);
  assert(refs[0].raw === '_evo/core/workflows/brainstorming/workflow.md', `Wrong raw[0]: ${refs[0].raw}`);
  assert(refs[1].raw === '_evo/core/workflows/party-mode/workflow.md', `Wrong raw[1]: ${refs[1].raw}`);
});
...
test('minimal.csv: extracts refs from minimal 3-column CSV', () => {
  ...
  assert(refs[0].raw === '_evo/core/tasks/help.md', `Wrong raw: ${refs[0].raw}`);
