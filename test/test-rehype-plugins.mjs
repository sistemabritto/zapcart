...
function runTests() {
  ...
  {
    const tree = makeAnchorTree('page.md');
    transform(tree, STD_FILE, { ...STD_OPTS, base: '/EVO-METHOD/' });
    assert(getHref(tree) === '/EVO-METHOD/guide/page/', 'Base /EVO-METHOD/ -> /EVO-METHOD/guide/page/', `Got ${getHref(tree)}`);
  }

  {
    const tree = makeAnchorTree('page.md');
    transform(tree, STD_FILE, { ...STD_OPTS, base: '/EVO-METHOD' });
    assert(getHref(tree) === '/EVO-METHOD/guide/page/', 'Base /EVO-METHOD (no trailing slash) -> same result', `Got ${getHref(tree)}`);
  }
  ...
  {
    const tree = makeAnchorTree('/page/');
    transformBase(tree, { base: '/EVO-METHOD/' });
    assert(getHref(tree) === '/EVO-METHOD/page/', 'Base /EVO-METHOD/ prefixes', `Got ${getHref(tree)}`);
  }

  {
    const tree = makeAnchorTree('/page/');
    transformBase(tree, { base: '/EVO-METHOD' });
    assert(getHref(tree) === '/EVO-METHOD/page/', 'Base /EVO-METHOD normalizes (adds trailing slash)', `Got ${getHref(tree)}`);
  }
...

  {
    ...
    assert(getHref(tree) === '/EVO-METHOD/page/', 'a[href] prefixed', `Got ${getHref(tree)}`);
  }

  {
    const tree = makeElementTree('img', { src: '/img/logo.png' });
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/EVO-METHOD/img/logo.png', 'img[src] prefixed', `Got ${getSrc(tree)}`);
  }

  {
    ...
    transformBase(tree, { base: BASE });
    assert(
      tree.children[0].properties.href === '/EVO-METHOD/styles/main.css',
      'link[href] prefixed',
      `Got ${tree.children[0].properties.href}`,
    );
  }
...

  {
    ...
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/EVO-METHOD/media/intro.mp4', 'video[src] prefixed', `Got ${getSrc(tree)}`);
  }

  {
    const tree = makeElementTree('audio', { src: '/media/clip.mp3' });
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/EVO-METHOD/media/clip.mp3', 'audio[src] prefixed', `Got ${getSrc(tree)}`);
  }

  {
    const tree = makeElementTree('iframe', { src: '/embed/widget' });
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/EVO-METHOD/embed/widget', 'iframe[src] prefixed', `Got ${getSrc(tree)}`);
  }
...

  {
    ...
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/EVO-METHOD/media/alt.mp4', 'source[src] prefixed', `Got ${getSrc(tree)}`);
  }
  ...
  {
    const tree = makeAnchorTree('/EVO-METHOD/page/');
    transformBase(tree, { base: BASE });
    assert(getHref(tree) === '/EVO-METHOD/page/', 'Already prefixed skipped', `Got ${getHref(tree)}`);
  }
...

  {
    ...
    transformBase(tree, { base: BASE });
    assert(getRawValue(tree) === '<img src="/EVO-METHOD/img/logo.png">', 'Raw img src rewritten', `Got ${getRawValue(tree)}`);
  }

  {
    ...
    transformBase(tree, { base: BASE });
    assert(getRawValue(tree) === '<a href="/EVO-METHOD/page/">link</a>', 'Raw a href rewritten', `Got ${getRawValue(tree)}`);
  }
...

  {
    const tree = {
      ...
      children: [
        {
          type: 'raw',
          value: '<img src="/EVO-METHOD/img/logo.png">',
        },
      ],
    };
    transformBase(tree, { base: BASE });
    assert(getRawValue(tree) === '<img src="/EVO-METHOD/img/logo.png">', 'Raw already prefixed unchanged', `Got ${getRawValue(tree)}`);
  }

  {
    ...
    transformBase(tree, { base: BASE });
    assert(
      getRawValue(tree) === '<a href="/EVO-METHOD/page/"><img src="/EVO-METHOD/img/logo.png"></a>',
...

  {
    ...
    transformBase(tree, { base: BASE });
    const href = getHref(tree);
    assert(href === '/EVO-METHOD/guide/sibling/', './sibling.md through both -> no double prefix', `Got ${href}`);
  }

  {
    ...
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/EVO-METHOD/img/logo.png', 'img /img/logo.png -> only base-paths prefixes', `Got ${getSrc(tree)}`);
  }
...

  {
    ...
    transform(tree, STD_FILE, { ...STD_OPTS, base: BASE });
    transformBase(tree, { base: BASE });
    assert(getHref(tree) === '/EVO-METHOD/page/', '/page/ (non-.md) -> only base-paths prefixes', `Got ${getHref(tree)}`);
  }
